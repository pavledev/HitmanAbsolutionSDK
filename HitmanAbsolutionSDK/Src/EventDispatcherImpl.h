...
template<class... Args> class EventDispatcherImpl : public EventDispatcher<Args...>
{
  ...
  protected:
    void AddListenerInternal(void* p_Context, void* p_Listener) override
    {
        ...
        auto* registration = new EventDispatcherBase::EventListenerRegistration();
        registration->m_Listener = p_Listener;
        registration->m_Context = p_Context;

        m_Listeners.insert(m_Listeners.end() - 1, registration);
