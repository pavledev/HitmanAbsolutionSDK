...
class IMorphemeCutSequenceAnimatable
{
  public:
    ...
    virtual bool
    SetCutSequenceData(ISequenceEntity* sequenceEntity, const SCutSequenceData& cutSequenceData, const ZString& string, float param4) = 0;
...
class alignas(16) ZHM5BaseCharacter : public ZEntityImpl,
                                      ...
                                      public IBoneCollidable
{
  public:
    ...
    virtual bool CanPlayAnim(
        const SMatrix& mStart, unsigned int iAnimNode, unsigned int* apBoneList, unsigned int iNumBones, float* apFractions,
        unsigned int iNumFractions, float* pfFractionFailed
    ) = 0;
    virtual bool IsOnPathFinder(unsigned int* apBoneList, unsigned int iNumBones, float fHeightOffset) const = 0;
    virtual void SampleAnim(
        unsigned int iAnimNode, unsigned int* apBoneList, unsigned int iNumBones, float* apFractions, unsigned int iNumFractions,
        SAnimSample& AnimSample
    ) = 0;
  ...
  private:
    PAD(0x10);                             // 0x18
    ZEntityRef m_OutfitEntity;             // 0x28
    PAD(0x18);                             // 0x2C
    TEntityRef<ZGeomEntity> m_rGeomEntity; // 0x44
    PAD(0x18);                             // 0x4C
    TEntityRef<IHM5Outfit> m_OutfitWorn;   // 0x64
    PAD(0x184);                            // 0x6C
};
