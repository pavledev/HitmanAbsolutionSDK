#pragma once

#include "ZHM5BaseCharacter.h"
#include "ZPhysics.h"
#include "TMaxArray.h"
#include "ZCharacterTemplateAspect.h"

enum EActorCCPreset
{
    ACCP_CivilianFemale = 0,
    ACCP_CivilianeMale = 1,
    ACCP_GuardMale = 2,
    ACCP_GuardMaleElite = 3,
    ACCP_GuardMaleSuperElite = 4,
    ACCP_BossDexter = 5,
    ACCP_BossSanchez = 6,
    ACCP_BossLenny = 7,
    ACCP_Ignore = 8,
    ACCP_None = 9
};

enum EActorDeathType
{
    eADT_UNDEFINED = 0,
    eADT_INVISIBLE = 1,
    eADT_ACCIDENT = 2,
    eADT_PACIFIED = 3,
    eADT_PACIFIEDACCIDENT = 4,
    eADT_MURDER = 5,
    eADT_BLOODY_MURDER = 6
};

enum EActorType
{
    eAT_Civilian = 0,
    eAT_Guard = 1,
    eAT_Hitman = 2,
    eAT_Last = 3
};

struct SActorSoundDefs
{
    enum EDefinition
    {
        _NoSound = 0,
        Dth = 1,
        Dth_BrkNck = 2,
        Dth_Fll = 3,
        Dth_GnSht = 4,
        Dth_HdSht = 5,
        Dth_Mpct = 6,
        Dth_SltThrt = 7,
        Dth_strngl = 8,
        Dth_Xplo = 9,
        Dth_PrpF = 10,
        Dth_Electro = 11,
        Dth_Burn = 12,
        Dth_Crush = 13,
        Gen_GrtHMCiv = 14,
        Gen_GrtHMCop = 15,
        Gen_GrtHMElite = 16,
        Gen_GrtHMGuard = 17,
        Gen_GtHlp = 18,
        Gen_GtHlpLd = 19,
        Gen_Hrt = 20,
        Gen_Mssng = 21,
        Gen_HMHere = 22,
        Gen_HMThere = 23,
        Gen_RdoDrm = 24,
        Gen_Srps = 25,
        Gen_SrpsLd = 26,
        Gen_SrpsGrabFt = 27,
        Gen_SrpsGrabBk = 28,
        Gen_StndRsp = 29,
        Gen_StopLd = 30,
        ClsCmbt_Ack = 31,
        ClsCmbt_Tnt = 32,
        Cmbt_BackupCll = 33,
        Cmbt_BackupRsp = 34,
        Cmbt_BadDsg = 35,
        Cmbt_Beg = 36,
        Cmbt_CllOut = 37,
        Cmbt_ClsAck = 38,
        Cmbt_CvrFirLdr = 39,
        Cmbt_CvrFirRsp = 40,
        Cmbt_Fire = 41,
        Cmbt_FireLdr = 42,
        Cmbt_GtHit = 43,
        Cmbt_GtHitCiv = 44,
        Cmbt_HitHM = 45,
        Cmbt_HMClsCmbtAck = 46,
        Cmbt_HMClsCmbtPrpAck = 47,
        Cmbt_HMCvr = 48,
        Cmbt_HMFire = 49,
        Cmbt_HMFlnk = 50,
        Cmbt_HMHeadPopr = 51,
        Cmbt_HMKll = 52,
        Cmbt_HMKllCiv = 53,
        Cmbt_HMKllName = 54,
        Cmbt_HMKllPrpTss = 55,
        Cmbt_HMKllXplo = 56,
        Cmbt_HMMssTnt = 57,
        Cmbt_HMRelod = 58,
        Cmbt_HMShrpShtr = 59,
        Cmbt_HMSht2Blw = 60,
        Cmbt_HMSpttd = 61,
        Cmbt_HMTrwXplo = 62,
        Cmbt_HMVnshd = 63,
        Cmbt_Hold = 64,
        Cmbt_HoldLdr = 65,
        Cmbt_HumShldFrndFir = 66,
        Cmbt_HumShldRls1 = 67,
        Cmbt_HumShldRls2 = 68,
        Cmbt_HumShldRls3 = 69,
        Cmbt_HumShldRlsFem1 = 70,
        Cmbt_HumShldRlsFem2 = 71,
        Cmbt_HumShldRlsFem3 = 72,
        Cmbt_HumShldVctm = 73,
        Cmbt_HumShldLdr = 74,
        Cmbt_HumShldBrth = 75,
        Cmbt_KllHM = 76,
        Cmbt_LngLst = 77,
        Cmbt_LngLstRsp = 78,
        Cmbt_LstMnStn = 79,
        Cmbt_LstSght = 80,
        Cmbt_LstSghtRsp = 81,
        Cmbt_NdrAttck = 82,
        Cmbt_Relod = 83,
        Cmbt_Scrm = 84,
        Cmbt_TkDwnLdr = 85,
        Cmbt_VntAck = 86,
        Cmbt_Whmp = 87,
        Cmbt_StalemateHold = 88,
        Cmbt_StaleMateTnt = 89,
        AvoidXplo = 90,
        AvoidXplo_Blw = 91,
        AvoidXplo_Stnd = 92,
        Ar_BadDsg = 93,
        Ar_BadDsgRpt = 94,
        Ar_BlmeKll = 95,
        Ar_CivRptHMKnownAck = 96,
        Ar_ChngDsg = 97,
        Ar_ChngDsgRpt = 98,
        Ar_DrgBody = 99,
        Ar_DrgBodyRpt = 100,
        Ar_FkeSrrdrTnt = 101,
        Ar_HMCmply = 102,
        Ar_HMDoor = 103,
        Ar_HMSecTp = 104,
        Ar_HMSecTpRpt = 105,
        Ar_HMFseBx = 106,
        Ar_HMFseBxRpt = 107,
        Ar_KyCrd = 108,
        Ar_KyCrdRpt = 109,
        Ar_RunAway = 110,
        Ar_RptX = 111,
        Ar_Trspss = 112,
        Ar_TrspssRpt = 113,
        Ar_WeapWrn1 = 114,
        Ar_WeapWrn2 = 115,
        Ar_WeapWrnMelee1 = 116,
        Ar_WeapWrnMelee2 = 117,
        Ar_Wrn1 = 118,
        Ar_Wrn2 = 119,
        Ar_Wrn3 = 120,
        InCa_BackupCll = 121,
        InCa_BackupRun = 122,
        InCa_ChckCvr = 123,
        InCa_ChckNdrCr = 124,
        InCa_CivCmnd = 125,
        InCa_CivRptAck = 126,
        InCa_CivRptFail = 127,
        InCa_CivUpset = 128,
        InCa_ClstTnt = 129,
        InCa_HMAsCivAck = 130,
        InCa_HMAsCopAck = 131,
        InCa_HMAsGrdAck = 132,
        InCa_HMAsEliteAck = 133,
        InCa_HMSpttd = 134,
        InCa_HMTnt = 135,
        InCa_Idle = 136,
        InCa_NitiateHMKnwn = 137,
        InCa_SrchLdr = 138,
        InCa_SrchRsp = 139,
        InCa_Stnd = 140,
        InCa_StndAgtd = 141,
        InCa_StndAgtdLdr = 142,
        InCa_StndAgtdHMKnwn = 143,
        InCa_StndAgtdHMKnwnLdr = 144,
        InCa_StndHMKnwn = 145,
        InCa_StndHMKnwnLdr = 146,
        InCa_StndLdr = 147,
        InCa_VntTnt = 148,
        InCa_ClsCmbtAck = 149,
        InCa_ClsCmbtInv = 150,
        InCa_ClsCmbtInvLdr = 151,
        InCa_ClsCmbtInvRsp = 152,
        InCa_ClsCmbtLdr = 153,
        InCa_ClsCmbtRpt = 154,
        InCa_ClsCmbtRsp = 155,
        InCa_DthAck = 156,
        InCa_DthInv = 157,
        InCa_DthInvLdr = 158,
        InCa_DthInvRsp = 159,
        InCa_DthLdr = 160,
        InCa_DthRpt = 161,
        InCa_DthRsp = 162,
        InCa_XploAck = 163,
        InCa_XploInv = 164,
        InCa_XploInvLdr = 165,
        InCa_XploInvRsp = 166,
        InCa_XploLdr = 167,
        InCa_XploRpt = 168,
        InCa_XploRsp = 169,
        InCa_XploSeeAck = 170,
        InCa_XploSeeInv = 171,
        InCa_XploSeeInvLdr = 172,
        InCa_XploSeeInvRsp = 173,
        InCa_XploSeeLdr = 174,
        InCa_XploSeeRpt = 175,
        InCa_XploSeeRsp = 176,
        InCa_GnShtAck = 177,
        InCa_GnShtInv = 178,
        InCa_GnShtInvLdr = 179,
        InCa_GnShtInvRsp = 180,
        InCa_GnShtLdr = 181,
        InCa_GnShtRsp = 182,
        InCa_GnShtRpt = 183,
        InCa_ScrmAck = 184,
        InCa_ScrmInv = 185,
        InCa_ScrmInvLdr = 186,
        InCa_ScrmInvRsp = 187,
        InCa_ScrmLdr = 188,
        InCa_ScrmRpt = 189,
        InCa_ScrmRsp = 190,
        InCa_GenAck = 191,
        InCa_GenInv = 192,
        InCa_GenInvLdr = 193,
        InCa_GenInvRsp = 194,
        InCa_GenLdr = 195,
        InCa_GenRpt = 196,
        InCa_GenRsp = 197,
        InCa_GenLdAck = 198,
        InCa_GenLdInv = 199,
        InCa_GenLdInvLdr = 200,
        InCa_GenLdInvRsp = 201,
        InCa_GenLdLdr = 202,
        InCa_GenLdRpt = 203,
        InCa_GenLdRsp = 204,
        Accdnt_Inv = 205,
        Accdnt_Wtch = 206,
        InDedBdy_BloodPllAck = 207,
        InDedBdy_Ack = 208,
        InDedBdy_Inv = 209,
        InDedBdy_Rpt = 210,
        InDedBdy_BllPllRpt = 211,
        InDedBdy_Massive = 212,
        InDedBdy_PcfdInv = 213,
        InDsg_Ack = 214,
        InDsg_FllwWrn1 = 215,
        InDsg_FllwWrn2 = 216,
        InDsg_FllwWrn3 = 217,
        InDsg_Pzzl = 218,
        InDsg_Stnd = 219,
        InDsg_HdNPlnSght = 220,
        Trspss_Stnd = 221,
        Trspss_Wrn1 = 222,
        Trspss_Wrn2 = 223,
        Trspss_Wrn3 = 224,
        Trspss_Rsp = 225,
        InCu_CivRsp = 226,
        InCu_BlltMpctAck = 227,
        InCu_BlltMpctInv = 228,
        InCu_BlltMpctLdr = 229,
        InCu_BlltMpctRsp = 230,
        InCu_BlltMpctStnd = 231,
        InCu_BlltMpctStndRsp = 232,
        InCu_CrAlrmAck = 233,
        InCu_CrAlrmInv = 234,
        InCu_CrAlrmLdr = 235,
        InCu_CrAlrmRsp = 236,
        InCu_CrAlrmStnd = 237,
        InCu_CrAlrmStndRsp = 238,
        InCu_FtStpsAck = 239,
        InCu_FtStpsStnd = 240,
        InCu_NpcBrkAck = 241,
        InCu_NpcBrkCiv = 242,
        InCu_NpcBrkInv = 243,
        InCu_NpcBrkStnd = 244,
        InCu_PrpTssHearAck = 245,
        InCu_PrpTssHearInv = 246,
        InCu_PrpTssHearLdr = 247,
        InCu_PrpTssHearRsp = 248,
        InCu_PrpTssHearStnd = 249,
        InCu_PrpTssHearStndRsp = 250,
        InCu_PrpTssSeeAck = 251,
        InCu_PrpTssSeeCiv = 252,
        InCu_PrpTssSeeInv = 253,
        InCu_PrpTssSeeLdr = 254,
        InCu_PrpTssSeeRsp = 255,
        InCu_PrpTssSeeStnd = 256,
        InCu_PrpTssSeeStndRsp = 257,
        InCu_RdoAck = 258,
        InCu_RdoInv = 259,
        InCu_RdoLdr = 260,
        InCu_RdoRsp = 261,
        InCu_RdoStnd = 262,
        InCu_RdoStndRsp = 263,
        InCu_GuardStnd = 264,
        InSt_Ack = 265,
        InSt_HMAglty = 266,
        InSt_HMBz = 267,
        InSt_HMBzStnd = 268,
        InSt_HMEntXit = 269,
        InSt_HMInCvr = 270,
        InSt_HMRun = 271,
        InSt_HMSnkng = 272,
        InSt_PrpTssSee = 273,
        InSt_Stnd = 274,
        InSt_Wrn = 275,
        InSt_HM2Cls = 276,
        FseBx_Fixed = 277,
        FseBx_Fixing = 278,
        FseBx_GoFix = 279,
        FseBx_SabAck = 280,
        _Last = 281
    };
};

class ZHM5CCProfile : public ZEntityImpl
{
  public:
    static ZHM5CCProfile* GetDefaultCCProfile(EActorCCPreset p_ActorCCPreset)
    {
        if (p_ActorCCPreset >= ACCP_None)
        {
            return 0;
        }

        return Globals::PresetCCProfiles[p_ActorCCPreset];
    }

    int32_t m_nAttacksForTakeDown;
    float m_fTakeDownWindow;
    float m_fTakeDownWindowIncFraction;
    float m_fHitmanDamage;
    bool m_bAllowAttackingHitman;
    float m_fDecayTime;
    int32_t m_nKillMashNum;
    int32_t m_nNumChainsToKill;
    EActorCCPreset m_eActorCCPreset;
};

class ZActor : public ZHM5BaseCharacter, public ICharacterCollision
{
  public:
    void KillActor(EActorDeathType p_DeathType, bool p_DeathVisible)
    {
        Functions::ZActor_KillActor->Call(this, p_DeathType, p_DeathVisible);
    }

    float4 GetBoneWorldPosition(uint32_t p_BoneId) const
    {
        float4 result;
        Functions::ZActor_GetBoneWorldPosition->Call(this, result, p_BoneId);

        return result;
    }

    TEntityRef<ZHM5CCProfile> m_rCCProfile;                    // 0x1F4
    PAD(0x10);                                                 // 0x1FC
    ZString m_sActorName;                                      // 0x20C
    EActorType m_eActorType;                                   // 0x214
    PAD(0x1C);                                                 // 0x218
    TArray<TEntityRef<IHM5Item>> m_inventory;                  // 0x234
    PAD(0x64);                                                 // 0x240
    TEntityRef<ZCharacterTemplateAspect> m_pCharacterTemplate; // 0x2A4
    PAD(0x910);                                                // 0x2AC
    TArray<TEntityRef<IHM5Item>> m_runtimeInventory;           // 0xBBC
    PAD(0x38);                                                 // 0xBC8
};

static_assert(sizeof(ZActor) == 0xC00);

class ZActorManager : public IComponentInterface
{
  public:
    PAD(0x19668);                                    // 0x4
    TMaxArray<TEntityRef<ZActor>, 50> m_aliveActors; // 0x1966C
    PAD(0x97D0);                                     // 0x19800
};

static_assert(sizeof(ZActorManager) == 0x22FD0);
